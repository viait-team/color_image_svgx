...
def merge_tsv(new_tsv_path, orig_tsv_path, tsv_path):
    # Load original TSV
    with open(orig_tsv_path, newline='', encoding='utf-8') as f:
        reader = csv.DictReader(f, delimiter='\t')
        orig_rows = list(reader)
        fieldnames = reader.fieldnames

    # Merge logic: loop new_tsv first
    with open(new_tsv_path, newline='', encoding='utf-8') as f:
        reader = csv.DictReader(f, delimiter='\t')
        for new_row in reader:
            new_text = new_row['text'].strip()
            new_level = int(new_row['level'])
            new_conf = float(new_row['conf'])

            if new_conf < 50.0:
                continue  # Skip low-confidence text
            if not is_number(new_text) or new_level != 5:
                continue  # Only process numeric text
            ...
            for orig_row in orig_rows:
                orig_left = int(orig_row['left'])
                orig_top = int(orig_row['top'])
                orig_level = int(orig_row['level'])

                if orig_left == new_left and orig_top == new_top and orig_level == 5:
                    matched = True
                    if not orig_row['text'].strip():
                        orig_row['text'] = new_text
                        orig_row['conf'] = new_row['conf']
                        orig_row['width'] = new_row['width']
                        orig_row['height'] = new_row['height']
                    break

            if not matched:
                orig_rows.append(new_row)

    # Write updated TSV back to original file
    with open(tsv_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames, delimiter='\t')
        writer.writeheader()
        writer.writerows(orig_rows)

    print(f"Merged TSV written to: {tsv_path}")
...
def run_ocr_replace_text_path(image_file, svg_file, output_stem):
    ...
    if is_it_a_table:
        ...
        # Tesseract automatically adds the .tsv extension to the output stem
        command = f'"{TESSERACT_PATH}" "{image_file}" "{output_stem}_orig" -l eng --psm 12 tsv'
        process_command(command)
        tsv_orig_file = f"{output_stem}_orig.tsv"
        verbose(f"Successfully created '{tsv_file}'")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Tesseract automatically adds the .tsv extension to the output stem
            command = f'"{TESSERACT_PATH}" "{bilevel_image_file_path}" "{output_stem}_bilevel" -l eng --psm 6 tsv'
            process_command(command)
            tsv_bilevel_file = f"{output_stem}_bilevel.tsv"
            verbose(f"bilevel Successfully created '{tsv_bilevel_file}'")

            merge_tsv(tsv_bilevel_file, tsv_orig_file, tsv_file)
            delete_file(tsv_bilevel_file)
            delete_file(tsv_orig_file)
...
def color_trace_sequential(args):
    ...
    for input_file, output_file in get_inputs_outputs(args.input, output_pattern):
        ...
        is_it_a_table = is_table_image(path=input_file)
