...
class SVGXTsvProcessor {
    ...
    _mutateTSV(tsvText, useRelaxedRule = false) {
        ...
        let numericTotal = 0;
        let twoDecimalCount = 0;
        let mutatedCount = 0;

        // Helper: transform integer to float with decimal before last 2 digits
        function transformNumber(str) {
            const sign = str.startsWith('-') || str.startsWith('+') ? str[0] : '';
            const digits = str.replace(/^[-+]/, '');
            if (digits.length < 3) return str; // skip short numbers
            const head = digits.slice(0, -2);
            const tail = digits.slice(-2);
            return sign + (head || '0') + '.' + tail;
        }

        // First pass: scan for percent and decimal stats
        ...
        if (shouldMutate) {
            for (let i = 1; i < lines.length; i++) {
                lines[i] = lines[i].replace(/O/g, '0');
            }
        }

        // Second pass: apply mutation if needed
        const mutatedLines = lines.map((line, i) => {
            if (i === 0 || !shouldMutate) return line;

            const cols = line.split('\t');
            let text = cols[textIndex];
            if (!text) return line;
        ...
        console.log(`- "percent" found: ${percentFound}`);
        console.log(`- Numeric entries: ${numericTotal}`);
        console.log(`- With two decimals: ${twoDecimalCount}`);
        console.log(`- Ratio: ${(decimalRatio * 100).toFixed(2)}%`);
        console.log(`- Mutated entries: ${mutatedCount}`);

        return mutatedLines.join('\n');
    }

    // with floating point fixed
    geCorrectedTsvContent() {
    ...
    GetImageText(useRelaxedRule = false) {
        ...
        try {
            ...
            const filteredGroups = allFinalGroups.filter(group => {
                const isShort = group.text.length < 2;
